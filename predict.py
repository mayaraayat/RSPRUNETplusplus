...
class Predict:
    ...
    def get_prob(self, image: np.ndarray) -> np.ndarray:
        """
        Predict on an image.
    ...
    def predict(self, image: np.ndarray) -> np.ndarray:
        ...
        Returns:
            np.ndarray: Predictions.
        """
        outputs = self.get_prob(image)

        pred_array, best_prob_array = self.postprocess_output(outputs)
        return pred_array, best_prob_array

    def get_prob_cubes(self, cubes: list[np.ndarray]) -> np.ndarray:
        """
    ...
    def predict_cubes(self, cubes: np.ndarray) -> np.ndarray:
        ...
        Returns:
            np.ndarray: Predictions.
        """
        probs = self.get_prob_cubes(cubes)
    ...
    def predict_image_file(
        ...
        if h < limit_im_size and w < limit_im_size:
            ...
            print('Predicting ...')
            prob_cubes = self.get_prob_cubes(cubes)
